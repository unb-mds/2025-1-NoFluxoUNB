...
def get_or_create_curso(nome_curso, matriz_json, periodo_letivo):
    ...
    result = executar_operacao(
        ...
        .eq("matriz_curricular", periodo_letivo)
        .execute
    )
...
def verificar_estrutura_curricular(arquivo_json):
    ...
    result_materias_banco = executar_operacao(
        supabase.table("materias_por_curso")
        .select("id_materia_curso, id_materia, nivel")
        .eq("id_curso", id_curso)
        .execute
    )
    ...
    for nivel_data in dados_curso["niveis"]:
        ...
        for materia in materias:
            ...
            if id_materia in materias_banco and materias_banco[id_materia] == nivel_num:
                print(
                    f"    ✓ {codigo}: {materia['nome']} - Já existe no nível {nivel_num}"
                )
                materias_ignoradas += 1
            else:
                # Associa a matéria ao curso
                resultado = get_or_create_materia_por_curso(
                    id_materia, id_curso, nivel_num
                )
                if resultado:
                    print(
                        f"    ➕ {codigo}: {materia['nome']} - Inserida no nível {nivel_num}"
                    )
...
def main():
    ...
    for arquivo in arquivos_json:
        ...
        try:
            processadas, inseridas, ignoradas = verificar_estrutura_curricular(
                caminho_completo
            )
