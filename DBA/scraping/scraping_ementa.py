...
# Retentativas em caso de SSL/conexão (ex.: SIGAA fechando a conexão)
MAX_RETRIES = 4
RETRY_DELAY_SEC = 12
...
def extract_prazos_cargas(relatorio_html):
    ...
    for th in soup.find_all("th"):
        ...
        if (
            "total m" in t_lower
            and "nima" in t_lower
            and "optativa" not in t_lower
            and "complementar" not in t_lower
            and "obrigat" not in t_lower
        ):
            out["total_minima"] = val
        elif "subtotal de ch de aula" in t_lower:
            out["subtotal_ch_aula"] = val
        elif (
            "subtotal de ch de orienta" in t_lower or "orientação acadêmica" in t_lower
        ):
...
def _parse_estrutura_celula(texto):
    ...
    m = re.match(
        r"^([A-Z0-9]+)\s*-\s*(.+?)\s*-\s*(\d+)h\s*$", texto, re.IGNORECASE | re.DOTALL
    )
    if m:
...
def extract_dados_por_nivel(relatorio_html):
                ...
                while j < len(all_trs):
                    ...
                    if "componentes" in (tr_comp.get("class") or []):
                        ...
                        if len(tds) >= 2:
                            ...
                            if parsed:
                                natureza = _extrair_natureza_da_celula(
    ...
    if not niveis:
        ...
        for idx in range(1, len(blocos), 2):
            ...
            conteudo = re.split(
                r"cadeia|grupo de componentes", conteudo, flags=re.IGNORECASE
            )[0]
            materias = []
            for m in re.findall(
                r"([a-z0-9]+)\s*-\s*(.*?)\s*-\s*(\d+)h", conteudo, flags=re.IGNORECASE
            ):
                ...
                materias.append(
                    {
                        ...
                        "natureza": (
                            "Obrigatória"
                            if "nivel" in nome_nivel.lower()
                            and "optativa" not in nome_nivel.lower()
                            else "Optativa"
                        ),
                    }
                )
        ...
        match_optativas = re.search(
            r"(optativas[\w\s]*)(.*)", texto, flags=re.IGNORECASE
        )
        if match_optativas:
            conteudo_opt = match_optativas.group(2)
            conteudo_opt = re.split(
                r"cadeia|grupo de componentes", conteudo_opt, flags=re.IGNORECASE
            )[0]
            materias_opt = []
            for m in re.findall(
...
def acessar_relatorio(session, soup, btn_name, btn_value, estrutura_id, estrutura_url):
    ...
    resp = request_with_retry(session, "post", post_url, data=form_data)
    return resp.text
...
def scrape_estruturas():
    ...
    # Cursos que devem ter TODAS as estruturas curriculares (exceção; demais usam MAX_RELATORIOS_POR_CURSO)
    # Incluir com e sem acento: SIGAA pode vir "CIÊNCIAS AMBIENTAIS " ou "CIENCIAS AMBIENTAIS"
    CURSOS_ESTRUTURAS_ILIMITADAS = {
    ...
    for idx, tr in enumerate(linhas_curso):
        ...
        print(
            f"[{idx + 1}/{len(linhas_curso)}] Processando: {nome_curso} ({turno or 'sem turno'})"
        )
        print(f"DEBUG: Link do curso: {curso_url}")

        resp = request_with_retry(session, "get", curso_url)
        soup = BeautifulSoup(resp.text, "html.parser")
        ...
        if not estrutura_link:
            print(
                f"Erro: Não encontrado link para estrutura curricular em {curso_url}."
            )
            continue
        ...
        resp = request_with_retry(session, "get", estrutura_url)
        soup = BeautifulSoup(resp.text, "html.parser")

        linhas = soup.find_all("tr", class_=["linha_impar", "linha_par"])
        relatorios = []
        MAX_RELATORIOS_POR_CURSO = 4  # pegar até 4 estruturas curriculares por curso
        nome_curso_norm = normalize(
            nome_curso
        )  # strip + lower (ex: "CIÊNCIAS AMBIENTAIS " -> "ciências ambientais")
        pegar_todas_estruturas = nome_curso_norm in CURSOS_ESTRUTURAS_ILIMITADAS
        if pegar_todas_estruturas:
            print(f"Exceção: coletando TODAS as estruturas para o curso '{nome_curso}'")
        for tr in linhas:
            if (
                not pegar_todas_estruturas
                and len(relatorios) >= MAX_RELATORIOS_POR_CURSO
            ):
                break
            tds = tr.find_all("td")
            if len(tds) >= 2:
                ...
                for txt in tds:
                    ...
                    match_periodo = re.search(
                        r"Período letivo em vigor:?\s*([0-9]{4}\.[12])", txt.text
                    )
                    if match_periodo:
                        periodo_letivo = match_periodo.group(1)
                for a in tr.find_all(
                    "a", title="Relatório da Estrutura Curricular", onclick=True
                ):
                    onclick = a["onclick"]
                    ...
                    if btn_match and id_match:
                        ...
                        id_arquivo = (
                            periodo_letivo
                            if periodo_letivo
                            else re.sub(r"[^\w]", "_", texto_identificador)
                        )
                        relatorios.append(
                            (
                                btn_name,
        ...
        for idx, (
            ...
            print(
                f"Relatório encontrado e baixado para: {nome_curso} - {id_arquivo if id_arquivo else f'estructura{idx+1}'}"
            )
            ...
            th = relatorio_soup.find(
                "th", string=lambda s: s and "Período Letivo de Entrada em Vigor" in s
            )
            if th:
                td = th.find_next_sibling("td")
                if td:
            ...
            nome_arquivo = (
                periodo_letivo_vigor
                if periodo_letivo_vigor
                else (id_arquivo if id_arquivo else f"estructura{idx+1}")
            )
            ...
            prazos_merged["carga_horaria_maxima_componentes_eletivos"] = (
                campos_regex_curriculo["carga_horaria_maxima_componentes_eletivos"]
            )
