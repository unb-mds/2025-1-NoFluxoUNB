...
def request_with_retry(session, method, url, max_retries=MAX_RETRIES, delay=RETRY_DELAY_SEC, **kwargs):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt == max_retries - 1:
                raise
            print(f"Erro de conexão/SSL (tentativa {attempt + 1}/{max_retries}): {type(e).__name__}. Aguardando {delay}s...")
            time.sleep(delay)
    raise last_error
...
def scrape_estruturas():
    ...
    CURSOS_ESTRUTURAS_ILIMITADAS = {
        ...
        'ciências ambientais', 'ciencias ambientais',
    }
    ...
    for idx, tr in enumerate(linhas_curso):
        ...
        nome_curso_norm = normalize(nome_curso)  # strip + lower (ex: "CIÊNCIAS AMBIENTAIS " -> "ciências ambientais")
        ...
        for tr in linhas:
            if not pegar_todas_estruturas and len(relatorios) >= MAX_RELATORIOS_POR_CURSO:
                break
            tds = tr.find_all('td')
            if len(tds) >= 2:
                texto_identificador = ''
