...
def scrape_estruturas():
    ...
    for curso in cursos:
        ...
        menu_ensino = soup.find(
            "span", {"class": "item-menu"}, string=lambda s: s and "Ensino" in s
        )
        if menu_ensino:
            ...
            if sub_menu:
                for a in sub_menu.find_all("a", href=True):
                    if (
                        "estruturaCurricular.jsf" in a["href"]
                        or "curriculo.jsf" in a["href"]
                    ):
        ...
        if relatorio_link:
            ...
            print(
                f"Não encontrado link do relatório para: {nome_curso}. Extraindo dados direto da página de estrutura curricular."
            )
    ...
    for tr in soup.find_all("tr"):
        ...
        if len(tds) == 1 and "Nível" in tds[0].text:
            ...
            if len(dados) >= 3:
                ...
                carga = dados[-1]
                materias.append(
                    {"codigo": codigo, "nome": nome, "carga_horaria": carga}
                )
    # Salva o último nível
    if nivel_atual and materias:
        niveis.append(
            {"nivel": nivel_atual, "natureza": natureza_atual, "materias": materias}
        )

    # Salvar em JSON
    with open(output_json, "w", encoding="utf-8") as f:
        json.dump(
            {"curso": nome_curso, "niveis": niveis}, f, ensure_ascii=False, indent=2
        )
