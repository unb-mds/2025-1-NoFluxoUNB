...
def coleta_dados(session, component_id, viewstate, base_url, params):
    ...
    try:
            ...
            if "visualizacao" in table.get("class", []):
                ...
                for row in rows:
                    ...
                    if len(cells) >= 2:
                        ...
                        if (
                            "Equivalencias" in label
                            and "Historico de Equivalencias" not in label
                        ):
        ...
        return {
            "unidade_responsavel": (
                _remover_acentos(
                    details_raw.get("Unidade Responsavel", "Nao informado")
                )
            ).split("-")[0],
            "ementa": _remover_acentos(
                ementa_raw or details_raw.get("Ementa/Descricao", "Nao informado")
            ),
        }
...
def processar_departamento(id_atual):
    ...
    for tentativa in range(MAX_RETRIES):
        ...
        try:
            ...
            viewstate_form = soup.find("input", {"name": "javax.faces.ViewState"})[
                "value"
            ]
            buscar_id = soup.find("input", {"value": "Buscar"}).get(
                "id", "formTurma:j_id_jsp_1370969402_11"
            )
                    ...
                    if "agrupador" in row.get("class", []):
                        ...
                        if link:
                            ...
                            if id_match:
                                ...
                                viewstate_input = results_soup.find(
                        ...
                        if (
                            len(cols) >= 5
                            and current_component_name
                            and current_component_name
                            not in materias_com_turma_adicionada
                        ):
                            turma = {
                                "disciplina": (
                                    current_component_name.split("-")[1].strip()
                                    if "-" in current_component_name
                                    else current_component_name
                                ),
                                "codigo": (
                                    current_component_name.split("-")[0].strip()
                                    if "-" in current_component_name
                                    else "N/A"
                                ),
                            }
...
def main():
    ...
    for i in range(0, total_departamentos, lote_size):
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            futures = [
                executor.submit(processar_departamento, id_depto)
                for id_depto in lote_ids
            ]

            for future in tqdm(
                as_completed(futures), total=len(lote_ids), desc="Progresso do Lote"
            ):
    ...
    for resultado_depto in todos_dados_por_depto:
        for turma_info in resultado_depto["turmas"]:
            turma_com_depto_id = {
                "departamento_id": resultado_depto["departamento_id"],
                **turma_info,
            }
    ...
    if turmas_flat_list:
        try:
            ...
            csv_path = os.path.join(
                output_dir, f"turmas_unb_{time.strftime('%Y%m%d_%H%M%S')}_FULL.csv"
            )
