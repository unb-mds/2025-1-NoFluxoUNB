...
# Caminhos das pastas
PASTA_MATRIZES = os.path.join(
    os.path.dirname(__file__), "..", "dados", "estruturas-curriculares"
)
...
def parse_curriculo(curriculo_str):
    ...
    if "/" in s:
        ...
        if " - " in resto and re.search(r"\d{4}\.[12]\s*$", resto):
            ...
            matriz_ano = (
                parts[1].strip()
                if len(parts) > 1 and re.match(r"^\d{4}\.[12]$", parts[1].strip())
                else None
            )
        else:
            matriz_versao = resto
        ...
        if not codigo_curso_base or not codigo_curso_base.isdigit():
            return {
                "codigo_curso_base": None,
                "matriz_versao": None,
                "matriz_ano": None,
            }
        return {
            "codigo_curso_base": codigo_curso_base,
            "matriz_versao": matriz_versao,
...
def build_dados_matriz(curriculo_str, periodo_letivo_vigor):
    ...
    curriculo_completo = (
        f"{curriculo_str} - {periodo_letivo_vigor}".strip(" - ")
        if curriculo_str or periodo_letivo_vigor
        else ""
    )
...
def get_or_create_curso(codigo_base, nome_curso, tipo_curso):
    ...
    result = executar_operacao(
        ...
        .select("id_curso", "nome_curso", "tipo_curso")
...
def get_or_create_matriz(
    ...
    result = executar_operacao(
        ...
        .select("id_matriz")
        .eq("curriculo_completo", curriculo_completo)
        .execute
    )
    if result.data:
...
for arquivo in os.listdir(PASTA_TURMAS_DEPTO):
    if arquivo.startswith("turmas_depto_") and arquivo.endswith(".json"):
        with open(
            os.path.join(PASTA_TURMAS_DEPTO, arquivo), "r", encoding="utf-8"
        ) as f:
            turmas = json.load(f)
            for turma in turmas:
                ...
                depto = turma.get(
                    "unidade_responsavel",
                    turma.get("departamento", turma.get("depto", None)),
                )
                if cod and depto:
...
CACHE_ID_MATERIA = {}
try:
    res = executar_operacao(
        supabase.table("materias").select("codigo_materia", "id_materia").execute
    )
    for r in res.data or []:
        if r.get("codigo_materia"):
            CACHE_ID_MATERIA[r["codigo_materia"]] = r["id_materia"]
    print(
        f"[Cache] {len(CACHE_ID_MATERIA)} matérias carregadas (codigo -> id_materia)."
    )
except Exception:
    pass
...
def get_or_create_materia(materia):
    ...
    if result.data:
        ...
        if materia.get("ementa", "") and materia.get("ementa", "") != ementa_atual:
            executar_operacao(
                supabase.table("materias")
                .update({"ementa": materia.get("ementa", "")})
                .eq("id_materia", id_materia)
                .execute
            )
        if departamento and (
            not departamento_atual or departamento != departamento_atual
        ):
            executar_operacao(
                supabase.table("materias")
...
def get_or_create_materia_por_curso(id_materia, id_matriz, nivel):
    ...
    try:
        res = executar_operacao(
            supabase.table("materias_por_curso").insert(insert_data).execute
        )
        return (
            res.data[0].get("id_materia_curso") or res.data[0].get("id")
            if res.data
            else None
        )
    except Exception:
        ...
        try:
            ...
            if "23505" not in str(e) and "duplicate key" not in str(e).lower():
                for row in lote:
                    try:
                        executar_operacao(
                            supabase.table("materias_por_curso").insert(row).execute
                        )
                    except Exception:
                        pass
...
def _codigos_na_expressao(expr):
    ...
    if not expr or not isinstance(expr, str):
        return []
    return re.findall(r"[A-Z]{2,}\d{2,}[A-Z0-9]*", expr.upper()) or re.findall(
        r"[A-Z]+\d+", expr.upper()
    )
...
def _codigos_em_expressoes_mat_det(mat_det):
    ...
    for campo, key in [
        ...
        if isinstance(val, list):
            for item in val:
                expr = (
                    item.get(key, item.get("expressao", ""))
...
def ensure_materias_existem(codigos, materias_detalhadas_dict=None):
    ...
    for cod in codigos:
        ...
        if _id_materia_from_cache(cod) is not None:
            continue
        mat = (
            detalhes.get(cod) or detalhes.get(cod.upper()) or detalhes.get(cod.lower())
        )
        if isinstance(mat, dict):
            get_or_create_materia(
                {
                    ...
                    "codigo": cod,
                    "carga_horaria": ch_to_int(
                        mat.get("carga_horaria", mat.get("ch", 0))
                    ),
            ...
            get_or_create_materia(
                {"nome": cod, "codigo": cod, "carga_horaria": 0, "ementa": ""}
            )


def get_or_create_pre_requisito(
    id_materia, expressao_original, id_materia_requisito=None
):
    ...
    try:
        res = executar_operacao(
            supabase.table("pre_requisitos").insert(payload).execute
        )
        ...
        result = executar_operacao(
            ...
            .eq("id_materia", id_materia)
            .eq("expressao_original", expressao_original)
            .execute
        )
        return result.data[0]["id_pre_requisito"] if result.data else None


def get_or_create_co_requisito(
    id_materia, expressao_original, id_materia_corequisito=None
):
...
def get_or_create_equivalencia(
    id_materia, id_curso, expressao_original, curriculo=None, data_vigencia=None
):
...
def processar_matrizes():
    ...
    try:
        res_cursos = executar_operacao(
            supabase.table("cursos").select("id_curso", "nome_curso").execute
        )
    ...
    for i, arquivo in enumerate(os.listdir(PASTA_MATRIZES), 1):
        ...
        with open(os.path.join(PASTA_MATRIZES, arquivo), "r", encoding="utf-8") as f:
            matriz = json.load(f)
            nome_curso = (
                remover_acentos(matriz["curso"]).replace("Ç", "C").upper().strip()
            )
                ...
                for materia in nivel["materias"]:
                    ...
                    dados_materia = {
                        ...
                        "carga_horaria": ch_to_int(
                            mat_det.get(
                                "carga_horaria",
                                materia.get("carga_horaria", materia.get("ch", 0)),
                            )
                        ),
            ...
            id_materias_matriz = list(
                {
                    CACHE_ID_MATERIA.get(m["codigo"])
                    for nivel in matriz["niveis"]
                    for m in nivel["materias"]
                    if CACHE_ID_MATERIA.get(m["codigo"]) is not None
                }
            )
            pre_existentes = set()
            co_existentes = set()
            ...
            if id_materias_matriz:
                try:
                    r_pre = executar_operacao(
                        supabase.table("pre_requisitos")
                        .select("id_materia", "expressao_original")
                        .in_("id_materia", id_materias_matriz)
                        .execute
                    )
                    for row in r_pre.data or []:
                        pre_existentes.add(
                            (
                                row["id_materia"],
                                (row.get("expressao_original") or "").strip(),
                            )
                        )
                except Exception:
                    pass
                try:
                    r_co = executar_operacao(
                        supabase.table("co_requisitos")
                        .select("id_materia", "expressao_original")
                        .in_("id_materia", id_materias_matriz)
                        .execute
                    ...
                    pass
                try:
                    r_eq = executar_operacao(
                        supabase.table("equivalencias")
                        .select(
                            "id_materia", "expressao_original", "id_curso", "curriculo"
                        )
                        .in_("id_materia", id_materias_matriz)
                        .execute
                    )
                    for row in r_eq.data or []:
                        equiv_existentes.add(
                            (
                                row["id_materia"],
                                (row.get("expressao_original") or "").strip(),
                                row.get("id_curso"),
                ...
                for materia in nivel["materias"]:
                                ...
                                .insert(
                                    {
                                        ...
                                        "expressao_logica": build_expressao_logica(
                                            expr_eq
                                        ),
                                        "curriculo": None,
                                        "data_vigencia": None,
                    ...
                    if equiv_esp and isinstance(equiv_esp, list):
                        for eqesp in equiv_esp:
                            nome_curso_esp = remover_acentos(
                                eqesp.get("matriz_curricular", "")
                                .strip()
                                .split(" - ")[0]
                                .upper()
                            )
                            ...
                            if codigo_base_esp:
                                ...
                                if id_c is not None and any(
                                    c.get("id_curso") == id_c
                                    for c in CACHE_CURSOS_LISTA
                                ):
                                    id_curso_esp = id_c
                            if id_curso_esp is None and nome_curso_esp:
                                for curso in CACHE_CURSOS_LISTA:
                                    nome_curso_banco = remover_acentos(
                                        (curso.get("nome_curso") or "").strip().upper()
                                    )
                            ...
                            fim_vigencia_eq = (
                                periodo_letivo_to_date(fim_vigencia_raw)
                                if fim_vigencia_raw
                                else None
                            )
...
def processar_materias():
    ...
    lista_arquivos = sorted(
        [f for f in os.listdir(PASTA_MATERIAS) if f.endswith(".json")]
    )
    try:
        res_c = executar_operacao(
            supabase.table("cursos").select("id_curso", "nome_curso").execute
        )
    ...
    for idx, arquivo in enumerate(lista_arquivos, 1):
        print(
            f"\r[Matérias {idx}/{len(lista_arquivos)}] {arquivo[:40]}...",
            end="",
            flush=True,
        )
        with open(os.path.join(PASTA_MATERIAS, arquivo), "r", encoding="utf-8") as f:
                ...
                if equiv_esp and isinstance(equiv_esp, list):
                    for eqesp in equiv_esp:
                        ...
                        if codigo_base_esp:
                            id_c = to_int(codigo_base_esp)
                            if id_c is not None and any(
                                c.get("id_curso") == id_c for c in cache_cursos
                            ):
                                id_curso_esp = id_c
                        if id_curso_esp is None:
                            for curso in cache_cursos:
                                ...
                                if nome_curso_banco == nome_curso_esp:
                                    id_curso_esp = curso["id_curso"]
                                    break
                        data_vigencia_eq = periodo_letivo_to_date(
                            eqesp.get("data_vigencia", "").strip()
                        )
                        fim_vigencia_eq = (
                            periodo_letivo_to_date(
                                eqesp.get("fim_vigencia", "").strip()
                            )
                            if eqesp.get("fim_vigencia")
                            else None
                        )
                        expr_esp = eqesp.get("expressao", "").strip()
                        if (
...
def processar_uma_matriz(arquivo_path):
    ...
    if id_materias_matriz:
        try:
            ...
            for row in r_pre.data or []:
                pre_existentes.add(
                    (row["id_materia"], (row.get("expressao_original") or "").strip())
                )
        except Exception:
            pass
        try:
            ...
            if equiv_esp and isinstance(equiv_esp, list):
                for eqesp in equiv_esp:
                    ...
                    if codigo_base_esp:
                        id_c = to_int(codigo_base_esp)
                        if id_c is not None and any(
                            c.get("id_curso") == id_c for c in cache_cursos_lista
                        ):
                            id_curso_esp = id_c
                    if id_curso_esp is None and nome_curso_esp:
                        for curso in cache_cursos_lista:
    ...
    print(
        f"  Curso: {nome_curso} | Matriz: {dados_mat['curriculo_completo']} | id_matriz={id_matriz} | {len(linhas_matriz)} matérias."
    )
    return id_matriz
