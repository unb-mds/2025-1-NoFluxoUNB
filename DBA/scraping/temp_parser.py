...
def extract_dados_por_nivel(relatorio_html):
    ...
    nome_curso = (
        tabela_principal.find("th", text="Matriz Curricular:")
        .find_next("td")
        .text.strip()
    )
    codigo_curso = (
        tabela_principal.find("th", text="Código:").find_next("td").text.strip()
    )
            ...
            niveis.append(
                {
                    "nivel": nivel,
                    "natureza": (
                        "Obrigatória" if "Optativas" not in nivel else "Optativa"
                    ),
                    "materias": materias,
                }
            )
    ...
    for nivel in niveis:
        print(
            f"- {nivel['nivel']} ({nivel['natureza']}): {len(nivel['materias'])} matérias"
        )
    return niveis
