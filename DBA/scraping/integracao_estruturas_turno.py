...
SUPABASE_KEY = (
    _env_key
    if (_env_key and len(_env_key) > 50 and _env_key.startswith("eyJ"))
    else _SERVICE_KEY
)
...
if os.path.isdir(PASTA_MATERIAS):
    for arquivo in os.listdir(PASTA_MATERIAS):
        if arquivo.startswith("turmas_depto_") and arquivo.endswith(".json"):
            with open(
                os.path.join(PASTA_MATERIAS, arquivo), "r", encoding="utf-8"
            ) as f:
                for turma in json.load(f):
                    cod = turma.get("codigo", turma.get("codigo_materia"))
                    depto = turma.get(
                        "unidade_responsavel",
                        turma.get("departamento", turma.get("depto")),
                    )
                    if cod and depto:
...
def get_or_create_materia(materia):
    ...
    if not codigo:
        return None
    mat_det = materias_detalhadas.get(
        codigo,
        materias_detalhadas.get(
            codigo.upper(), materias_detalhadas.get(codigo.lower(), {})
        ),
    )
    dados = {
        ...
        "carga_horaria": ch_to_int(
            mat_det.get(
                "carga_horaria",
                mat_det.get("ch", materia.get("ch", materia.get("carga_horaria", 0))),
            )
        ),
    ...
    if result.data:
        ...
        if dados["ementa"] and dados["ementa"] != (row.get("ementa") or ""):
            executar_operacao(
                supabase.table("materias")
                .update({"ementa": dados["ementa"]})
                .eq("id_materia", id_m)
                .execute
            )
        if departamento and (
            not row.get("departamento") or row.get("departamento") != departamento
        ):
            executar_operacao(
                supabase.table("materias")
...
def listar_arquivos_turno():
    ...
    return [
        f
        for f in os.listdir(PASTA_ESTRUTURAS)
...
def processar_um_arquivo(caminho_arquivo):
    ...
    if not matriz:
        return None, 0
    nome_curso = (
        remover_acentos(matriz.get("curso", "")).replace("Ç", "C").upper().strip()
    )
...
def processar():
    ...
    for i, arquivo in enumerate(arquivos, 1):
        ...
        periodo_letivo = matriz.get("periodo_letivo_vigor")
        curriculo_str = matriz.get("curriculo") or ""
        turno = matriz.get("turno", "") or (
            "DIURNO"
            if " - diurno.json" in arquivo
            else "NOTURNO" if " - noturno.json" in arquivo else ""
        )
        codigo_base = extrair_codigo_base(curriculo_str)
        if not codigo_base:
    ...
    print(
        f"Matérias novas inseridas: {materias_criadas_depois - materias_criadas_antes}"
    )
