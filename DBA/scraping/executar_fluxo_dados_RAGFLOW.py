...
def gerar_nomes_pastas():
    ...
    nomes = {
        ...
        "formatados": os.path.join(
            diretorio_base, f"chunks_finais_formatados_{sufixo_semestre}"
        ),
    }
...
def executar_script(argumentos_comando):
    ...
    try:
        ...
        print(
            f"❌  ERRO: O script '{nome_do_script}' encontrou um erro e foi interrompido."
        )
        print("\n--- Erro Detalhado ---\n" + e.stderr + "\n----------------------")
        return False
...
def main():
    ...
    comandos = [
        [
            sys.executable,
            os.path.join(pasta_scripts, "extrair_turmas_sigaa.py"),
            pastas["dados_finais"],
        ],
        [
            sys.executable,
            os.path.join(pasta_scripts, "converter_json_para_txt.py"),
            pastas["dados_finais"],
            pastas["chunks"],
        ],
        [
            sys.executable,
            os.path.join(pasta_scripts, "formatar_para_ragflow.py"),
            pastas["chunks"],
            pastas["formatados"],
        ],
    ]
