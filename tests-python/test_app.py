...
def test_analisar_materia_endpoint_sucesso(client, mocker):
    ...
    mocker.patch(
        "no_fluxo_backend.ai_agent.app.gerar_texto_ranking",
        return_value="Ranking formatado com sucesso.",
    )
...
def test_analisar_materia_endpoint_bad_request(client, payload, erro_esperado):
    ...
    """
    response = client.post(
        "/assistente", data=json.dumps(payload), content_type="application/json"
    )
...
def test_analisar_materia_endpoint_erro_inesperado(client, mocker):
    ...
    """
    mocker.patch(
        "no_fluxo_backend.ai_agent.app.remover_acentos_nativo",
