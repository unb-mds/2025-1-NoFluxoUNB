...
class TestHelperFunctions(unittest.TestCase):
    ...
    def test_remover_acentos(self):
        ...
        self.assertEqual(
            scraper._remover_acentos("Olá, como vai você? CÉDULA"),
            "Ola, como vai voce? CEDULA",
        )
        self.assertEqual(
            scraper._remover_acentos("Engenharia de Computação"),
            "Engenharia de Computacao",
        )
        self.assertEqual(
            scraper._remover_acentos("Texto sem acentos"), "Texto sem acentos"
        )
        self.assertEqual(scraper._remover_acentos(""), "")
        self.assertEqual(
            scraper._remover_acentos(123), 123
        )  # Deve retornar o valor original se não for string
    ...
    def test_limpar_texto(self):
        ...
        self.assertEqual(
            scraper.limpar_texto(None), None
        )  # Deve lidar com valores não-string
...
class TestDataExtraction(unittest.TestCase):
    ...
    def test_extrair_equivalencias(self):
        ...
        self.assertEqual(
            resultado,
            "FGA0001 (COMPUTACAO BASICA), FGA0002 (ALGORITMOS E ESTRUTURAS DE DADOS)",
        )
    ...
    def test_coleta_dados(self, mock_extrair_equivalencias):
        ...
        dados = scraper.coleta_dados(
            mock_session, "12345", "viewstate_xyz", "http://fake.url", {}
        )
...
class TestMainProcess(unittest.TestCase):
    ...
    def test_carregar_ids_departamentos(self, mock_file):
        ...
        mock_file.assert_called_with(
            "dados/departamentos_ID_unb.csv", "r", encoding="utf-8"
        )
    ...
    def test_salvar_por_departamento(
        self, mock_mkdir, mock_join, mock_json_dump, mock_open_file
    ):
        ...
        resultados = [
            {"departamento_id": 650, "turmas": [{"codigo": "CIC0001"}]},
            {
                "departamento_id": 651,
                "turmas": [],
            },  # Departamento sem turmas não deve ser salvo
        ]
        ...
        mock_open_file.assert_called_once_with(
            "fake_dir/turmas_depto_650.json", "w", encoding="utf-8"
        )
