...
class TestFormatadorParaRagflow(unittest.TestCase):
    ...
    def test_processamento_basico_com_dois_blocos(
        self, mock_file, mock_listdir, mock_makedirs, mock_exists
    ):
        """
        ...
        mock_listdir.return_value = [
            "curso_engenharia.txt",
            "notas.md",
        ]  # Apenas .txt será processado
        ...
        script_a_testar.preprocess_disciplines_txt_from_folder(
            "pasta_entrada_fake", "pasta_saida_fake"
        )
        ...
        mock_file.assert_any_call(
            os.path.join("pasta_entrada_fake", "curso_engenharia.txt"),
            "r",
            encoding="utf-8",
        )
        ...
        mock_file.assert_any_call(
            os.path.join("pasta_saida_fake", "preprocessed_curso_engenharia.txt"),
            "w",
            encoding="utf-8",
        )
    ...
    @patch("builtins.open", new_callable=mock_open)
    def test_delimitadores_customizados(
        self, mock_file, mock_listdir, mock_makedirs, mock_exists
    ):
        """
    ...
    @patch(
        "builtins.open", mock_open(read_data="Apenas um texto qualquer sem o marcador.")
    )
    def test_arquivo_sem_disciplinas(self, mock_listdir, mock_makedirs, mock_exists):
        ...
        handle.assert_called_with(
            os.path.join("out", "preprocessed_arquivo_sem_disciplina.txt"),
            "w",
            encoding="utf-8",
        )
