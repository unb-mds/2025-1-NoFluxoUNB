...
class TestExecutarFluxoDadosRAGFLOW(unittest.TestCase):
    ...
    def test_executar_script_sucesso(self, mock_subprocess_run):
        mock_subprocess_run.return_value = MagicMock(
            stdout="Saída de teste", stderr="", returncode=0
        )

        resultado = executar_fluxo_dados_RAGFLOW.executar_script(
    ...
    def test_executar_script_erro_execucao(self, mock_subprocess_run):
        mock_subprocess_run.side_effect = (
            executar_fluxo_dados_RAGFLOW.subprocess.CalledProcessError(
                1, "cmd", stderr="Erro de execução"
            )
        )

        resultado = executar_fluxo_dados_RAGFLOW.executar_script(
            ["python", "script_com_erro.py"]
        )
        self.assertFalse(resultado)
        mock_subprocess_run.assert_called_once()
