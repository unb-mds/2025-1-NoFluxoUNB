<script lang="ts">
	...
	let { courseData, focusCode, onNavigate }: Props = $props();
...
{#if analysis}
	<div class="space-y-5">
		<section class="rounded-2xl border border-white/12 bg-black/25 p-4 sm:p-5">
			<p class="mb-2 inline-flex rounded-md border border-lime-300/35 bg-lime-400/10 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-lime-200">
				Matéria atual
			</p>
			{#if analysis.focusMateria}
				<h2 class="text-xl font-semibold text-white sm:text-3xl">{analysis.focusMateria.nomeMateria}</h2>
				<p class="mt-1 font-mono text-xs text-white/55 sm:text-sm">{analysis.focusMateria.codigoMateria}</p>
				<div class="mt-3 flex flex-wrap gap-2">
					<span class="rounded-md border border-white/15 bg-white/5 px-2.5 py-1 font-mono text-xs text-white/70">
		...
		<section class="rounded-2xl border border-white/10 bg-black/20 p-4 sm:p-5">
							...
							{#each col as m}
								<button
									...
									class="w-[162px] rounded-xl border px-3 py-2 text-left transition-colors {isFocusMateria(m, analysis.focusCode)
										? 'border-lime-300/45 bg-lime-400/14'
								...
								>
									<p class="font-mono text-xs {isFocusMateria(m, analysis.focusCode) ? 'text-lime-200' : 'text-cyan-200'}">
										{m.codigoMateria}
									</p>
		...
		<section class="overflow-hidden rounded-2xl border border-white/10 bg-black/20">
			<button
				type="button"
		...
		<section class="overflow-hidden rounded-2xl border border-white/10 bg-black/20">
			...
			{#if openEq}
				<div class="border-t border-white/10 px-4 py-3">
					{#if analysis.equivalencies.length === 0}
						<p class="text-xs text-white/45">Nenhuma encontrada nesta matriz.</p>
					{:else}
						<ul class="space-y-2">
							{#each analysis.equivalencies as eq}
