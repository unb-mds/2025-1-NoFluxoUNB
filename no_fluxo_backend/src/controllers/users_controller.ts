    ...
    routes: {
        "register-user-with-google": new Pair(RequestType.POST, async (req: Request, res: Response) => {
            ...
            if (userExistsError) {
                ...
                logger.error("Erro ao buscar usuário: ", userExistsError);
            ...
            if (userCreatedError) {
                logger.error("Erro ao criar usuário: ", userCreatedError);

                return res.status(500).json({ error: "Erro ao criar usuário" });
            }

            logger.info("Usuário criado com sucesso: ", userCreatedResult);
        ...
        "get-user-by-email": new Pair(RequestType.GET, async (req: Request, res: Response) => {
            ...
            logger.info("Buscando usuário por email: ", req.query);
