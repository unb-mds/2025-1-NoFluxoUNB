...
def extrair_dados_academicos(texto_total):
    ...
    # Adicionar IRA como item se encontrado (valor_texto = como no PDF)
    if ira:
    ...
    for i, linha in enumerate(linhas):
        ...
        if match_linha1 and i + 1 < len(linhas):
            ...
            if match_linha2:
                ...
                professor, carga_h, turma, freq, nota, mencao, situacao = (
                    match_linha2.groups()
                )
                ...
                print(
                    f"  -> Disciplina (formato original): {codigo} - {nome.strip()[:30]}... (Status: {situacao})"
                )
    ...
    if disciplinas_encontradas == 0:
        ...
        print(
            f"[DEBUG] Encontradas {len(sample_lines)} linhas com padrão alternativo nas primeiras 20:"
        )
        ...
        for i, linha in enumerate(linhas):
            ...
            if match_alt_linha1 and i + 1 < len(linhas):
                ...
                if match_alt_linha2:
                    ...
                    disciplinas_encontradas += 1
                    print(
                        f"  -> Disciplina (formato alternativo): {codigo} - {nome.strip()[:30]}... (Status: {situacao})"
                    )
                else:
        ...
        for page_num in range(len(pdf_document)):
            ...
            try:
                page_text = pytesseract.image_to_string(
                    image, config=custom_config, lang="por"
                )
                if page_text.strip():
                    texto_total += page_text + "\n"
                    logger.info(
                        f"Extracted {len(page_text)} characters from page {page_num + 1} using OCR"
                    )
                else:
    ...
    try:
        ...
        if not texto_total.strip():
            ...
            return (
                jsonify(
                    {
                        "error": "Nenhuma informação textual pôde ser extraída do PDF usando OCR. O PDF pode estar vazio, corrompido ou ter qualidade muito baixa."
                    }
                ),
                422,
            )
        ...
        return (
            jsonify(
                {
                    "error": f"Ocorreu um erro interno ao processar o PDF com OCR: {str(e)}"
                }
            ),
            500,
        )
