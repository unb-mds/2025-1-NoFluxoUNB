...
def log_memory_usage():
    ...
    try:
        ...
        log_message(
            f"Python: Memory usage: {memory_mb:.2f} MB, Active threads: {len(active_threads)}"
        )
...
def run_git_command_safely(command, cwd=None, timeout=30):
    ...
    try:
        ...
        if result.returncode != 0 and result.stderr.strip():
            log_message(
                f"Python: Git command failed: {' '.join(command)} - {result.stderr.strip()}"
            )
...
def setup_git_credential_helper(username, token, repo_path):
    ...
    try:
        ...
        subprocess.run(
            ["git", "config", "--local", "credential.helper", "store"], check=True
        )

        # Write credentials to git credential store
        credential_input = f"protocol=https\nhost=github.com\nusername={username}\npassword={token}\n\n"
        process = subprocess.Popen(
            ["git", "credential", "approve"], stdin=subprocess.PIPE, text=True
        )
    ...
    try:
        ...
        for file in mounted_files:
            if os.path.exists(os.path.join(repo_dir, file)):
                ...
                run_git_command_safely(
                    ["git", "update-index", "--assume-unchanged", file], cwd=repo_dir
                )

    except Exception as e:
        log_message(
            f"Python: Warning - Could not setup git ignore for mounted files: {e}"
        )
...
def cleanup_git_stashes(repo_dir):
    ...
    try:
        ...
        if stash_list and stash_list.returncode == 0:
            stash_count = (
                len(stash_list.stdout.strip().split("\n"))
...
def check_for_updates(repo_dir, branch):
    ...
    try:
        ...
        test_result = run_git_command_safely(
            ["git", "rev-parse", "--git-dir"], cwd=repo_dir
        )
        ...
        if fetch_result is None or fetch_result.returncode != 0:
            error_msg = (
                fetch_result.stderr
                if fetch_result and fetch_result.stderr
                else "Unknown error"
            )
            log_message(
                f"Python: Warning - Failed to fetch from origin: {error_msg.strip()}"
            )
            return False

        local_result = run_git_command_safely(
            ["git", "rev-parse", "HEAD"], cwd=repo_dir
        )
        remote_result = run_git_command_safely(
...
def pull_updates(repo_dir, branch):
    ...
    try:
        ...
        stash_result = run_git_command_safely(
            ["git", "stash", "push", "-m", "Auto-stash before update"], cwd=repo_dir
        )
        if stash_result and stash_result.returncode != 0:
            log_message(
                "Python: Warning - Could not stash changes, continuing with pull..."
            )

        # Pull the latest changes
        pull_result = run_git_command_safely(
            ["git", "pull", "origin", branch], cwd=repo_dir
        )
        if pull_result is None or pull_result.returncode != 0:
            ...
            reset_result = run_git_command_safely(
                ["git", "reset", "--hard", f"origin/{branch}"], cwd=repo_dir
            )
            if reset_result is None or reset_result.returncode != 0:
                log_message(
                    "Python: Warning - Could not update repository, continuing with existing files"
                )
            else:
...
def update_fork_repo(fork_path, branch, git_username=None, git_token=None):
    ...
    if not fork_path or not os.path.exists(fork_path):
        log_message(
            f"Python: Fork path {fork_path} does not exist. Skipping fork update."
        )
        return
    ...
    try:
        ...
        if git_username and git_token:
            config_backup = setup_git_credential_helper(
                git_username, git_token, fork_path
            )
        ...
        try:
            ...
            if "@" in fork_origin_url and "github.com" in fork_origin_url:
                ...
                clean_url = (
                    "https://github.com/" + fork_origin_url.split("github.com/")[1]
                )
                subprocess.run(
                    ["git", "remote", "set-url", "origin", clean_url], check=True
                )
            ...
            log_message(
                f"Python: Warning - Could not get fork's origin URL: {e.stderr.decode().strip() if e.stderr else str(e)}"
            )
            ...
            if git_username and git_token:
                ...
                raise Exception(
                    "No authentication provided and origin remote doesn't exist"
                )
        ...
        try:
            ...
            log_message(
                f"Python: Warning - Could not reset/pull main branch: {e.stderr if hasattr(e, 'stderr') else str(e)}"
            )
        ...
        try:
            ...
            subprocess.run(
                ["git", "remote", "set-url", "upstream", origin_url], check=True
            )
        ...
        log_message(
            f"Python: Updating fork's main branch with content from upstream's {branch} branch"
        )
        ...
        log_message(
            f"Python: Fork repository main branch updated successfully with content from {branch}"
        )
    ...
    try:
        if process and process.poll() is None:  # Check if process is running
            ...
            try:
                ...
                log_message(
                    "Python: Process didn't terminate gracefully, force killing..."
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Monitor and auto-update a git repository."
    )
    parser.add_argument(
        "--branch", default="main", help="Git branch to monitor (default: main)"
    )
    parser.add_argument(
        "--fork-location",
        help="Path to fork repository that should be updated when origin changes",
    )
    parser.add_argument(
        "--git-username",
        help="Git username for authentication (can also use GIT_USERNAME in .env)",
    )
    parser.add_argument(
        "--git-token",
        help="Git token/password for authentication (can also use GIT_TOKEN in .env)",
    )
    ...
    if not GIT_USERNAME and FORK_LOCATION:
        log_message(
            "Python: Warning - No git username provided. Set GIT_USERNAME in .env file or use --git-username"
        )
    if not GIT_TOKEN and FORK_LOCATION:
        log_message(
            "Python: Warning - No git token provided. Set GIT_TOKEN in .env file or use --git-token"
        )
    ...
    try:
        while True:
            ...
            if git_cleanup_counter >= GIT_CLEANUP_INTERVAL and os.path.exists(
                "/.dockerenv"
            ):
            ...
            if check_for_updates(REPO_DIR, BRANCH):
                log_message(
                    f"Python: New changes detected in branch {BRANCH}. Updating..."
                )
                ...
                if new_process is None:
                    log_message(
                        "Python: Process crashed and could not be restarted. trying again in 10 seconds..."
                    )
